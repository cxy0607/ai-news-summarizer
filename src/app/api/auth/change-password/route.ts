...
export async function POST(request: Request) {
  try {
    ...
    if (!currentPassword || !newPassword) return NextResponse.json({ success: false, error: '缺少字段' }, { status: 400 });
    ...
    if (!match) return NextResponse.json({ success: false, error: '当前密码错误' }, { status: 401 });
