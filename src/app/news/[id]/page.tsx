...
export default function NewsDetailPage() {
    ...
    const fetchNews = async () => {
      try {
        ...
        if (data.success && data.data) {
          const found = data.data.news.find((item: any) => item.id === id);
          if (found) {
            setNews(found);
            // 生成AI摘要
  ...
  // 保存到阅读历史
  const saveToHistory = (newsItem: any) => {
    if (typeof window !== 'undefined') {
      const history = JSON.parse(localStorage.getItem('readingHistory') || '[]');
      // 去重
      const newHistory = [newsItem, ...history.filter((item: any) => item.id !== newsItem.id)];
      // 限制历史记录数量
  ...
  const checkBookmarkStatus = (newsId: string) => {
    if (typeof window !== 'undefined') {
      ...
      setIsBookmarked(bookmarks.some((item: any) => item.id === newsId));
    }
  };
  ...
  const toggleBookmark = () => {
    ...
    if (typeof window !== 'undefined') {
      ...
      if (isBookmarked) {
        newBookmarks = bookmarks.filter((item: any) => item.id !== news.id);
      } else {
